...
import { Scrooge, DEX } from "../typechain-types";

describe("DEX", () => {
  // this.timeout(45000);

  let dexContract: DEX;
  let scroogeContract: Scrooge;
  ...
  async function deployNewInstance() {
    before("Deploying fresh contracts", async function () {
      ...
      const ScroogeContract = await ethers.getContractFactory("Scrooge");
      scroogeContract = await ScroogeContract.deploy();
      const scroogeAddress = await scroogeContract.getAddress();

      const DexContract = await ethers.getContractFactory(contractArtifact);
      dexContract = (await DexContract.deploy(scroogeAddress)) as DEX;
      const dexContractAddress = await dexContract.getAddress();

      await scroogeContract.approve(dexContractAddress, ethers.parseEther("100"));
      ...
      await scroogeContract.transfer(user2.address, ethers.parseEther("10"));
      await scroogeContract.transfer(user3.address, ethers.parseEther("10"));
    });
  }
  ...
  describe("Checkpoint 4: Trading", function () {
    ...
    describe("ethToToken()", function () {
      it("Should be able to send 1 Ether to DEX in exchange for _ $SCR", async function () {
        const dexContractAddress = await dexContract.getAddress();
      ...
      it("Should send less tokens after the first trade (ethToToken called)", async function () {
        const user2BalBefore = await scroogeContract.balanceOf(user2.address);
        console.log("\t", " 💵 User2 initial $SCR balance:", ethers.formatEther(user2BalBefore));
        ...
        const user2BalAfter = await scroogeContract.balanceOf(user2.address);
        console.log("\t", " 💵 User2 new $SCR balance:", ethers.formatEther(user2BalAfter));

        const user3BalBefore = await scroogeContract.balanceOf(user3.address);
        console.log("\t", " 💵 User3 initial $SCR balance:", ethers.formatEther(user3BalBefore));
        ...
        const user3BalAfter = await scroogeContract.balanceOf(user3.address);
        console.log("\t", " 💵 User3 new $SCR balance:", ethers.formatEther(user3BalAfter));

        console.log("\t", " 💵 Expecting User2 to have aquired more $SCR than User3...");
      ...
      it("Should transfer tokens to purchaser after trade", async function () {
        const user3_token_before = await scroogeContract.balanceOf(user3.address);
        console.log("\t", " 💵 User3 initial $SCR balance:", ethers.formatEther(user3_token_before));
        ...
        const user3_token_after = await scroogeContract.balanceOf(user3.address);
        console.log("\t", " 💵 User3 new $SCR balance:", ethers.formatEther(user3_token_after));

        const tokenDifferece = user3_token_after - user3_token_before;
        console.log("\t", " 🥉 Expecting user3's $SCR balance to increase by the correct amount...");
    ...
    describe("tokenToEth()", async () => {
      it("Should send 1 $SCR to DEX in exchange for _ $ETH", async function () {
        const dexContractAddress = await dexContract.getAddress();
        const Scrooge_bal_start = await scroogeContract.balanceOf(dexContractAddress);
        console.log("\t", " 💵 Initial Scroogecoin $SCR balance:", ethers.formatEther(Scrooge_bal_start));
        const dex_eth_start = await ethers.provider.getBalance(dexContractAddress);
        ...
        const Scrooge_bal_end = await scroogeContract.balanceOf(dexContractAddress);
        console.log("\t", " 💵 Final Scroogecoin $SCR balance:", ethers.formatEther(Scrooge_bal_end));
        ...
        // Checks that the balance of the DEX contract has decreased by 1 $SCR
        console.log("\t", " 🎈 Expecting the $SCR balance of the Scrooge contract to have increased by 1...");
        expect(await scroogeContract.balanceOf(dexContractAddress)).to.equal(
          Scrooge_bal_start + ethers.parseEther("1"),
        );
  ...
  describe("Checkpoint 5: Liquidity", async () => {
    describe("deposit()", async () => {
      ...
      it("Should increase liquidity in the pool when ETH is deposited", async function () {
        ...
        await scroogeContract.connect(user2).approve(dexContractAddress, ethers.parseEther("100"));
    ...
    // pool should have 5:5 ETH:$SCR ratio
    describe("withdraw()", async () => {
      deployNewInstance();
      it("Should withdraw 1 ETH and 1 $SCR when pool is at a 1:1 ratio", async function () {
        ...
        const userscroogeBalance = await scroogeContract.balanceOf(deployer.address);
        console.log("\t", " 💵 User's starting $SCR balance:", ethers.formatEther(userscroogeBalance));

        console.log("\t", " 🔽 Calling withdraw with with a value of 1 Eth...");
        const tx1 = await dexContract.withdraw(ethers.parseEther("1"));
        const userscroogeBalanceAfter = await scroogeContract.balanceOf(deployer.address);
        const tx1_receipt = await tx1.wait();

        const eth_out = await getEventValue(tx1_receipt, 2);
        const token_out = await getEventValue(tx1_receipt, 3);

        console.log("\t", " 💵 User's new $SCR balance:", ethers.formatEther(userscroogeBalanceAfter));
        console.log(
          "\t",
          " 🎈 Expecting the balance to have increased by 1",
          ethers.formatEther(userscroogeBalanceAfter),
        );
        expect(userscroogeBalance, "User $SCR balance shoud increase").to.equal(
          userscroogeBalanceAfter - ethers.parseEther("1"),
        );
        ...
        console.log("\t", " 🔽 Expecting the $SCR withdrawn emit to be 1...");
