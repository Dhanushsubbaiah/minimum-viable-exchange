...
const Dex: NextPage = () => {
  ...
  const { data: ScroogeInfo } = useDeployedContractInfo("Scrooge");
  const { address: connectedAccount } = useAccount();

  const { data: DEXBalloonBalance } = useScaffoldContractRead({
    contractName: "Scrooge",
  ...
  const { data: userScrooge } = useScaffoldContractRead({
    contractName: "Scrooge",
    functionName: "balanceOf",
    ...
    <>
      <h1 className="text-center mb-4 mt-5">
        <span className="block text-xl text-right mr-7">
          🎈: {parseFloat(formatEther(userScrooge || 0n)).toFixed(4)}
        </span>
        <span className="block text-xl text-right mr-7">
          💦💦: {parseFloat(formatEther(userLiquidity || 0n)).toFixed(4)}
        </span>
        <span className="block text-2xl mb-2">Minimum viable Exchange</span>
        <span className="block text-4xl font-bold">DEX </span>
      </h1>
        ...
        <div className="px-5 py-5">
          ...
          <div className="space-y-4 bg-base-100 shadow-lg shadow-secondary border-8 border-secondary rounded-xl py-5 p-8 m-8">
            <div className="flex flex-col text-center mt-2 mb-4 px-4">
              <span className="block text-3xl font-semibold mb-2">Scroogecoin</span>
              <span className="mx-auto">
                <Address size="xl" address={ScroogeInfo?.address} />
              </span>
            </div>
